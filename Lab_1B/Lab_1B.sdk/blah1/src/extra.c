...
//includes for interrupts for Lab1B
...
{
	XGpio_InterruptDisable(&Gpio, PUSHBUTTON_MASK);
	XGpio *GpioPtr = (XGpio *)CallbackRef;

	//IntrFlag = 1;

	xil_printf("FUCK YEAH\n");

	/* Clear the Interrupt */
	XGpio_InterruptClear(GpioPtr, PUSHBUTTON_MASK);
	XGpio_InterruptEnable(&Gpio, PUSHBUTTON_MASK);
...
int extra_method()
{
	...
	Status = XGpio_Initialize(&Gpio, XPAR_BTNS_DEVICE_ID);
	if (Status != XST_SUCCESS)
		return XST_FAILURE;

//
	/* Hook up interrupt service routine */
	XIntc_Connect(&sys_intc, PUSHBUTTON_INTERRUPT_ID,
				      (Xil_ExceptionHandler)GpioHandler, &Gpio);

	/* Enable the interrupt vector at the interrupt controller */
	XIntc_Enable(&sys_intc, PUSHBUTTON_INTERRUPT_ID);
...
////	/*
////		 * Connect the application handler that will be called when an interrupt
////		 * for the START Button occurs
////	*/
////
////	Status = XIntc_Connect(&sys_intc, IntrId,
